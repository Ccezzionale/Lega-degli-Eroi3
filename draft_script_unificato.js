...
let ruoli = new Set(), squadre = new Set();

function normalize(nome) {
  return nome.trim().toLowerCase();
}

function inviaPickAlFoglio(pick, fantaTeam, nome, ruolo, squadraSerieA, quotazione) {
...
function caricaGiocatori() {
  return fetch("giocatori_completo_finale.csv").then(res => res.text()).then(csv => {
...
function caricaPick() {
  return fetch("https://docs.google.com/spreadsheets/d/e/2PACX-1vTDKKMarxp0Kl7kiIWa-1X7jB-54KcQaLIGArN1FfR_X40rwAKVRgUYRGhrzIJ7SsKtUPnk_Cz8F0qt/pub?output=csv")
    .then(res => res.text()).then(csv => {
      ...
      let prossima = null;
      righe.forEach(r => {
        const [pick, fantaTeam, nomeGrezzo, ruolo, squadraSerieA, quotazione] = r.split(",");
        const nome = nomeGrezzo ? nomeGrezzo.trim() : "";
        const key = normalize(nome);
        const tr = document.createElement("tr");
        giocatoriScelti.add(key);
        tr.innerHTML = `<td>${pick}</td><td>${fantaTeam}</td><td>${nome}</td><td>${ruolo}</td><td>${squadraSerieA}</td><td>${quotazione}</td>`;
        if (!nome && !prossima) {
...
function popolaListaDisponibili() {
  ...
  Object.values(mappaGiocatori).forEach(({ nome, ruolo, squadra, quotazione }) => {
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${nome}</td><td>${ruolo}</td><td>${squadra}</td><td>${quotazione}</td>`;
...
window.addEventListener("DOMContentLoaded", () => {
  caricaGiocatori().then(caricaPick).then(popolaListaDisponibili);
});
